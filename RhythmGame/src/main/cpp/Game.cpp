...
void Game::tap(int64_t eventTimeAsUptime) {
    if (mGameState != GameState::Playing){
        ...
        int64_t nextClapWindowTimeMs = 500;
...
TapResult Game::getTapResult(int64_t tapTimeInMillis, int64_t tapWindowInMillis){
    ...
    int64_t windowCenter = (int64_t) std::round((double) tapTimeInMillis / tapWindowInMillis) * tapWindowInMillis;
    LOGI("windowCenter %d", windowCenter);
    
    if (windowCenter % (2*tapWindowInMillis) == 0) {
        LOGI("::Early");
        return TapResult::Early;
    }
    
    if (tapTimeInMillis <= windowCenter + kWindowCenterOffsetMs) {
        if (tapTimeInMillis >= windowCenter - kWindowCenterOffsetMs) {
            LOGI("::Success");
...
bool Game::setupAudioSources() {
    ...
    mClap = std::make_unique<Player>(mClapSource, 0.5f);
